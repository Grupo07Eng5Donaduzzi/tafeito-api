...
export class HateoasInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<unknown> {
    ...
    if (!listOptions && !itemOptions) return next.handle();
    ...
    return next.handle().pipe(
      map((data: unknown) => {
        ...
        return this.transformItem(data as Record<string, unknown> | null, itemOptions!);
      }),
    );
  }
  ...
  private transformList(
    ...
    return {
      data: data.map((item) => ({ ...item, _links: options.itemLinks(item) })),
      meta: { totalItems: total, itemsPerPage: limit, currentPage: page, totalPages },
      _links: {
  ...
  private transformItem(
    ...
    if (!item) return null;
    return { ...item, _links: options.itemLinks(item) };
  }
}
